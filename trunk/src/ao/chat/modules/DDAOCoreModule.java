        ...
        try {
            if (channel == ModuleChannel.CON) {
                if (command.compareTo(commands[0]) == 0 && args != null) {//invite
                    ...
                    msg = msg.substring(2);
                    chatbot.sendChannelMessage(NameFormat.format(args[0]), msg);
                    ...
                    msg = msg.substring(2);
                    chatbot.sendPrivateChannelMessage(NameFormat.format(args[0]), msg);
                    ...
                    for (String s : args) {
                        chatbot.leaveChannel(NameFormat.format(s));
                    }
